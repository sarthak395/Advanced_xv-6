  ...
  for (;;)
  {
    ...
    for (int i = 1; i < 5; i++)
    {
      ...
      while (cur != 0)
      {
        ...
        if (process->state == RUNNABLE && (1 + ticks - process->qentertime > 100)) // 100 is the limit wait time
        {
    ...
    if (chosenproc->state == RUNNABLE)
    {
      if ((chosenproc->allowedtime == 0) && (q < 4))
      {
        q++;
      } // move to higher queue if time is up
      chosenproc->queue = q;
